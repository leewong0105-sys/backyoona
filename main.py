...
st.markdown(
    ...
    .year-card {
        position: relative;
        background: white;
        border: 2px solid #F5DDE2;
        border-radius: 22px;
        padding: 15px 20px;
        margin-bottom: 15px;
        box-shadow: 0 8px 24px rgba(80, 50, 60, .07);
        overflow: hidden;
    }
    ...
    [data-testid="stPlotlyChart"] {
        ...
        border-radius: 25px;
        padding: 5px;
        box-shadow: 0 12px 32px rgba(80, 50, 60, .09);
    }
...
def calculate_rates(df):
    ...
    # 숫자 변환
    for col in set(total_cols + elderly_cols):
        data[col] = pd.to_numeric(
            data[col],
            errors="coerce",
        ).fillna(0)

    data["전체인구"] = data[total_cols].sum(axis=1)
    data["고령인구"] = data[elderly_cols].sum(axis=1)
...
def attach_rates(geojson, rates):
...
def create_map(geojson):
    ...
    for feature in geojson["features"]:
        feature["properties"]["단계"] = get_stage(
            feature["properties"]["고령화율"]
        )

    # 단계별 색상이 섞이지 않도록 계단식 색상 사용
    colorscale = [
        [0.00, COLORS[0]],
        [0.20, COLORS[0]],
        [0.20, COLORS[1]],
...
try:
    ...
    with st.spinner(
        ...
        latest_year, rates = calculate_rates(
            population
        )
